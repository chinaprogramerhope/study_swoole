...
(new class {
    ...
    public function __construct() {
        try {
            swoole_set_process_name(sprinrf('php-ps:%s', 'master'));
    ...
    public function CreateProcess($index = null) {
        $process = new swoole_process(function (swoole_process $worker) use ($index) {
            ...
            swoole_set_process_name(sprinf('php-ps: %s', index));
            for ($j = 0; $j < 16000; $j++) {
    ...
    public function checkMpid(&$woker) {
        if (!swoole_process::kill($this->mpid, 0)) {
            $worker->exit();
            // 这句提示, 实际是看不到的, 需要写到日志中
            echo "master process exited, i [{$woker['pid']}] also quit\n";
        }
    }
