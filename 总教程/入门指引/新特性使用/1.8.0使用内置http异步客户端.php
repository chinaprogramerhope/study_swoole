...
在PHP中也有纯PHP实现的Http客户端，如Guzzle，这些类库最大的问题是Http协议解析是由PHP代码实现的，PHP代码在这样场景下进行大量运算性能较差，而且还会占用大量内存。swoole_http_client是由C代码实现的，解析Http协议的性能是非常高的，内存占用也很少。

在解析gzip压缩后的HTML时，swoole_http_client的优势更为明显，它可以使用download方法，以很小的内存占用即可完成超大文件的下载。由于PHP层面没有提供zlib流式分段解压的支持，只能将Http Body全部放置到内存中，调用gzdecode一次性解压，而这会占用大量内存。


SSL支持
swoole_http_client支持SSL和TLS隧道加密的https网址，并且支持配置客户端证书。
<?php
